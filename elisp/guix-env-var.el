;;; guix-env-var.el --- Guix environment variables mode
...
(defun guix-env-var-comment-line-p ()
  ...
  (save-excursion
    (beginning-of-line)
    (re-search-forward guix-env-var-comment-line-regexp (line-end-position) t)))
...
(defun guix-env-var-prettify-variable ()
  ...
  (let ((env-beg (line-beginning-position))
    ...
    (when (not (guix-env-var-comment-line-p))
...
(defun guix-env-var-prettify-buffer (&optional buffer)
  ...
  (interactive)
  (let ((buffer (unless buffer (current-buffer))))
    (goto-char (point-min))
...
\\{guix-env-var-mode-map}"
  (when guix-env-var-enable-formatting
    ...
    (when guix-env-var-display-message
      (message "This buffer has been prettified by `guix-env-var-mode'."))))
